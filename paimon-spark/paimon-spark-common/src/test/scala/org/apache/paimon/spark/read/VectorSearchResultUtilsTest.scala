...
class VectorSearchResultUtilsTest extends AnyFunSuite {
  ...
  test("identify row id and score only fields") {
    ...
    assert(
      VectorSearchResultUtils.isRowIdScoreOnly(
        Seq(PaimonMetadataColumn.SEARCH_SCORE_COLUMN, PaimonMetadataColumn.ROW_ID_COLUMN)))
    assert(!VectorSearchResultUtils.isRowIdScoreOnly(Seq.empty))
    assert(!VectorSearchResultUtils.isRowIdScoreOnly(Seq(PaimonMetadataColumn.ROW_ID_COLUMN, "id")))
  }
