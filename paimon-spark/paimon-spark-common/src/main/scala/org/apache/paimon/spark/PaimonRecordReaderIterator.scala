...
import org.apache.paimon.spark.schema.PaimonMetadataColumn.{PARTITION_AND_BUCKET_META_COLUMNS, PATH_AND_INDEX_META_COLUMNS, ROW_ID_COLUMN, VECTOR_SEARCH_SCORE_COLUMN}
...
case class PaimonRecordReaderIterator(
  ...
  Preconditions.checkArgument(
    !needScoreMetadata ||
      metadataColumns.forall(c => c.name == ROW_ID_COLUMN || c.name == VECTOR_SEARCH_SCORE_COLUMN))
