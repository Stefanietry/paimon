...
object VectorSearchResultUtils {
  ...
  private val RowIdScoreColumns = Set(
    PaimonMetadataColumn.ROW_ID_COLUMN,
    PaimonMetadataColumn.SEARCH_SCORE_COLUMN)

  def isRowIdScoreOnly(fieldNames: Seq[String]): Boolean = {
    fieldNames.nonEmpty && fieldNames.forall(RowIdScoreColumns.contains)
  }
  ...
  def toRows(result: GlobalIndexResult, requiredSchema: StructType): Array[InternalRow] = {
    ...
    result.results().iterator().asScala.map {
      rowId =>
        new GenericInternalRow(fieldNames.map(valueOf(_, rowId, scoreGetter)).asInstanceOf[Array[Any]])
    }.toArray
  }
