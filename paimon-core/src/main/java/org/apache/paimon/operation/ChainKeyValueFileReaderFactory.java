...
/** A specific implementation for {@link KeyValueFileReaderFactory} on chain table. */
public class ChainKeyValueFileReaderFactory extends KeyValueFileReaderFactory {
    ...
    private final String snapshotBranchName;

    private final String deltaBranchName;
    ...
    public ChainKeyValueFileReaderFactory(
        ...
        this.snapshotBranchName = super.schema.options().get(SCAN_FALLBACK_SNAPSHOT_BRANCH.key());
        this.deltaBranchName = super.schema.options().get(SCAN_FALLBACK_DELTA_BRANCH.key());
        this.snapshotSchemaManager =
                snapshotBranchName.equalsIgnoreCase(currentBranch)
                        ? super.schemaManager
                        : super.schemaManager.copyWithBranch(snapshotBranchName);
        this.deltaSchemaManager =
                deltaBranchName.equalsIgnoreCase(currentBranch)
                        ? super.schemaManager
                        : super.schemaManager.copyWithBranch(deltaBranchName);
    }
    ...
    public SchemaManager getSchemaManager(String fileName) {
        if (snapshotBranchName.equalsIgnoreCase(fileBranchMapping.get(fileName))) {
