...
/** Loads persisted native vector training models from global index files. */
class NativeVectorTrainingModelLoader {

    private NativeVectorTrainingModelLoader() {}
    ...
    public static VectorTrainingModel load(byte[] fileBytes) throws IOException {
        VectorGlobalIndexFileMeta meta = VectorGlobalIndexFileMeta.deserialize(fileBytes);
        byte[] payload = nativePayload(fileBytes);
        return VectorTrainingModelProviderRegistry.loadModel(
                meta.indexType(), meta.modelOptions(), payload);
    }
