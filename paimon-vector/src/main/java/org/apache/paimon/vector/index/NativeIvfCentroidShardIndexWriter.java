...
/** Writes one physical IVF vector index file for one centroid. */
class NativeIvfCentroidShardIndexWriter implements GlobalIndexSingleColumnWriter, Closeable {
    ...
    private final NativeVectorTrainingModelProvider.NativeVectorTrainingModel trainingModel;
    ...
    NativeIvfCentroidShardIndexWriter(
            GlobalIndexFileWriter fileWriter,
            NativeVectorTrainingModelProvider.NativeVectorTrainingModel trainingModel,
            int centroid,
    ...
    public List<ResultEntry> finish() {
        ...
        try {
            ...
            try (VectorIndexWriter writer = trainingModel.createCentroidShardWriter(centroid)) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(tempVectorFile, "r");
            ...
            while (remaining > 0) {
                ...
                if (thisBatch == addBatchSize) {
                    trainingModel.addCentroidVectors(writer, batchIds, batchVectors, thisBatch);
                } else {
                    trainingModel.addCentroidVectors(
                            writer,
